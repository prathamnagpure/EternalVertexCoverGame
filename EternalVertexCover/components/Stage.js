...
export default class Stage extends Component{
    constructor(props){
        ...
        for (const element of children) {
            if (element.type == 'node_stmt') {
                ...
                this.nodes.push({ id: String(element.node_id.id), pos: { x: parseFloat(coordinates[0]), y: parseFloat(coordinates[1]) } });
                ...
                this.nodesMap.get(String(element.edge_list[0].id)).neighbours.push(String(element.edge_list[1].id));
                this.nodesMap.get(String(element.edge_list[1].id)).neighbours.push(String(element.edge_list[0].id));
            }
        }
    }
    ...
    changeTurn = () => {
        if (this.state.turn == turns.defenderFirst){
            ...
            if (this.attackedEdge == undefined){
                ...
                if (!this.nodesMap.get(node1).ref.state.guardPresent && !this.nodesMap.get(node2).ref.state.guardPresent){
            ...
            nodeGuardCounter = new Map();
            this.nodesMap.forEach((node, node_id) => {
                nodeGuardCounter.set(node_id, (this.nodesMap.get(node_id).ref.state.guardPresent?1:0));
            });
            ...
            if (good){
                ...
                let [attackedNode1, attackedNode2] = this.attackedEdge.props.id.split(";");
                if (!this.nodesMap.get(attackedNode1).ref.state.guardPresent && !this.nodesMap.get(attackedNode2).ref.state.guardPresent){
                    Alert.alert("Game Over, Attacker Won");
                } else {
                    this.edgesMap.forEach((edge, edge_id) => {
                        edge.setState({ moveGuard1: false, moveGuard2: false, isAttacked: false})
                    });
                    this.setState({ turn: turns.attacker });
    ...
    showGuard = (node) => {
        if (this.state.turn == turns.defenderFirst){
            ...
            if (this.selected){
                if (this.nodesMap.get(this.selected.props.id).neighbours.includes(node.props.id)){
                    ...
                    if(this.edgesMap.has(edgeId)){
                        ...
                        this.edgesMap.get(node.props.id + ';' + this.selected.props.id).setState({moveGuard2: true})
                    }
                }
   ...
   selected = undefined;
    ...
    render = () => {
        let heading = <Text style={[this.styles.heading, {color: (this.state.guardCount < 0)? 'red' : 'white'}]}>Guards Left: {this.state.guardCount}</Text>
        if (this.state.turn == turns.attacker){
