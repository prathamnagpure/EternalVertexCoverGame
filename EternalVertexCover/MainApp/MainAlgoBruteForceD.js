...
//   console.log(turn, guards, attackedge, adjList, edgList,curMove);
  ...
  if (turn == 1) {
    ...
    //console.log("defender");
      let retPosits = [];
    if(dMoveMap.get(tupleToString(guards)+';'+attackedge+';'+curMove)!= undefined)
    {
        // console.log("HIT");
      ...
      let winWhere = undefined;
      let puraWhere = undefined;
      ...
      if (pos1 == -1) {
        ...
        for(let i =0;i <guards.length;i+=1) retPosits.push(i);

        dMoveMap.set(tupleToString(guards)+';'+attackedge+';'+curMove, [guards,0 , 1, result.lose,1,retPosits]);
          ...
          return [guards,0, 1, result.lose,1,retPosits];
            ...
            {
                for(let i =0;i <guards.length;i+=1) retPosits.push(i);
            let newpuraWhere=retPosits.map((element,index) => {var guardElem = guards[element];var newPos = sortedTuple.indexOf(guardElem);return newPos;})

                dMoveMap.set(tupleToString(guards)+';'+attackedge+';'+curMove, [guards,1,0,result.win,1,newpuraWhere]);
                return dMoveMap.get(tupleToString(guards)+';'+attackedge+';'+curMove);
            }
          cantgo[node1] = 1;
          cantgo[node2] = 0;
          ...
          retPosits = forDefender(guardsCopy, adjList, cantgo, 0).map(element => {return [element[0].concat([node1]),element[1].map(elementx => {return guardMp.get(elementx);}).concat([pos2])]});
        ...
        //console.log("no guard on right");
          ...
          retPosits = forDefender(guardsCopy, adjList, cantgo, 0).map(element => {return [element[0].concat([node2]),element[1].map(elementx => {return guardMp.get(elementx);}).concat([pos1])]});
          //console.log("postions' returned ");
          //console.log(retPosits);
          ...
          retPosits = forDefender(guardsCopy, adjList, cantgo, 0).map(element => {return [element[0].concat([node2,node1]),element[1].map(elementx => {return guardMp.get(elementx);}).concat([pos1,pos2])]});
          //console.log("postions' returned ");
          //console.log(retPosits);
          ...
          adjList[node1].forEach(element => {
            if (cantgo[element] == 0) {
              cantgo[element] = 1;
              retPosits=retPosits.concat(forDefender(guardsCopy, adjList, cantgo, 0).map(element2 => {return [element2[0].concat([element,node1]),element2[1].map(elementx => {return guardMp.get(elementx);}).concat([pos1,pos2])]})
);
              cantgo[element] = 0;
            }
          });
          ...
          adjList[node2].forEach(element => {
            if (cantgo[element] == 0) {
              cantgo[element] = 1;
              retPosits=retPosits.concat(forDefender(guardsCopy, adjList, cantgo, 0).map(element2 =>{return [element2[0].concat([element,node2]),element2[1].map(elementx => {return guardMp.get(elementx);}).concat([pos2,pos1])]}));
              cantgo[element] = 0;
            }
          });
          //console.log("postions' after concat ");
      ...
      let countt=0;
        ...
        if (pura == result.lose) {
            let newpuraWhere=puraWhere.map((element,index) => {var guardElem = guards[element];var newPos = sortedTuple.indexOf(guardElem);return newPos;})
          dMoveMap.set(tupleToString(guards)+';'+attackedge+';'+curMove, [puraState, dWin, dLose, pura,mxMoves,newpuraWhere]);
        } else if (pura == result.win) {
            let newwinWhere=winWhere.map((element,index) => {var guardElem = guards[element];var newPos = sortedTuple.indexOf(guardElem);return newPos;})
          dMoveMap.set(tupleToString(guards)+';'+attackedge+';'+curMove, [winState, dWin, dLose, pura,minMoves,newwinWhere]);
        } else {
          dMoveMap.set(tupleToString(guards)+';'+attackedge+';'+curMove, [puraState, dWin, dLose, pura, minMoves,puraWhere]);
        }
        return dMoveMap.get(tupleToString(guards)+';'+attackedge+';'+curMove);
    }
};

function giveMap(guardNum,guardPos,adjList,edgList,moves){
